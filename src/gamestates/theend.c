...
void Gamestate_Draw(struct Game *game, struct dosowiskoResources* data) {
	  al_draw_bitmap(data->bitmap, 0, 0, 0);
		...
		DrawTextWithShadow(game->data->font, al_map_rgb(255,255,255), 320/2, 20, ALLEGRO_ALIGN_CENTER, "Score:");
		...
		if (game->data->mediator.score == data->score) {
			DrawTextWithShadow(game->data->font, al_map_rgb(255,222, 120), 320/2, 140, ALLEGRO_ALIGN_CENTER, text);
		} else {
			DrawTextWithShadow(game->data->font, al_map_rgb(255,255,255), 320/2, 140, ALLEGRO_ALIGN_CENTER, text);
		}
		DrawTextWithShadow(game->data->font, al_map_rgb(255,255,255), 320/2, 162, ALLEGRO_ALIGN_CENTER, "Press RETURN");
}
