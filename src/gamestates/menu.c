...
void About(struct Game *game, struct MenuResources* data) {
	  ALLEGRO_TRANSFORM trans;
		...
		if (!game->data->font_bsod) {
			  game->data->font_bsod = al_create_builtin_font();
		}
		...
		al_draw_filled_rectangle(al_get_display_width(game->display)/2 - al_get_text_width(game->data->font_bsod, header)/2 - 4, (int)(al_get_display_height(game->display) * 0.32), 4 + al_get_display_width(game->display)/2 + al_get_text_width(game->data->font_bsod, header)/2, (int)(al_get_display_height(game->display) * 0.32) + al_get_font_line_height(game->data->font_bsod), al_map_rgb(170,170,170));

		al_draw_text(game->data->font_bsod, al_map_rgb(0, 0, 170), al_get_display_width(game->display)/2, (int)(al_get_display_height(game->display) * 0.32), ALLEGRO_ALIGN_CENTRE, header);

		char *header2 = "A fatal exception 0xD3RP has occured at 0028:M00F11NZ in GST SD(01) +";

		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2, (int)(al_get_display_height(game->display) * 0.32+2*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_CENTRE, header2);
		al_draw_textf(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2 - al_get_text_width(game->data->font_bsod, header2)/2, (int)(al_get_display_height(game->display) * 0.32+3*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_LEFT, "%p and system just doesn't know what went wrong.", (void*)game);

		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2, (int)(al_get_display_height(game->display) * 0.32+5*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_CENTRE, 	"About screen not implemented!");
		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2, (int)(al_get_display_height(game->display) * 0.32+7*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_CENTRE, 	"Made in the theater by Sebastian Krzyszkowiak and Konrad Burandt");

		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2 - al_get_text_width(game->data->font_bsod, header2)/2, (int)(al_get_display_height(game->display) * 0.32+9*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_LEFT, "* Press any key to terminate this error.");
		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2 - al_get_text_width(game->data->font_bsod, header2)/2, (int)(al_get_display_height(game->display) * 0.32+10*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_LEFT, "* Press any key to destroy all muffins in the world.");
		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2 - al_get_text_width(game->data->font_bsod, header2)/2, (int)(al_get_display_height(game->display) * 0.32+11*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_LEFT, "* Just kidding, please press any key anyway.");

		al_draw_text(game->data->font_bsod, al_map_rgb(255,255,255), al_get_display_width(game->display)/2, (int)(al_get_display_height(game->display) * 0.32+13*al_get_font_line_height(game->data->font_bsod)*1.25), ALLEGRO_ALIGN_CENTRE, "Press any key to continue _");
...
void ChangeMenuState(struct Game *game, struct MenuResources* data, enum menustate_enum state) {
	  data->menustate=state;
		...
		if (state == MENUSTATE_HIDDEN) {
			  ...
			  al_set_sample_instance_gain(game->data->muzyczka.instance.fg, 1.5);
		}
...
void Gamestate_Draw(struct Game *game, struct MenuResources* data) {
	  ...
	  al_set_target_bitmap(al_get_backbuffer(game->display));
		...
		if ((data->menustate == MENUSTATE_HIDDEN) && (!data->starting)) {
			  DrawTextWithShadow(game->data->font, al_map_rgb(255,255,255), game->viewport.width*0.5, game->viewport.height*0.9, ALLEGRO_ALIGN_CENTRE, "Press RETURN");
		}
...
void StartGame(struct Game *game, struct MenuResources *data) {
	  ...
	  game->data->mediator.lives = 3;
		game->data->mediator.score = 0;
		    game->data->mediator.modificator = 0.9756;
...
void Gamestate_Start(struct Game *game, struct MenuResources* data) {
	  ...
	  al_ungrab_mouse();
		...
		ChangeMenuState(game,data,MENUSTATE_HIDDEN);
		al_play_sample_instance(game->data->muzyczka.instance.fg);
		al_play_sample_instance(game->data->muzyczka.instance.bg);
		al_play_sample_instance(game->data->muzyczka.instance.drums);
		al_set_sample_instance_gain(game->data->muzyczka.instance.fg, 0.0);
		al_set_sample_instance_gain(game->data->muzyczka.instance.bg, 1.5);
		al_set_sample_instance_gain(game->data->muzyczka.instance.drums, 1.5);
